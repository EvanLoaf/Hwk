...
function smartSort() { // Сортировка массива согласно заданию
    switch (myArray.length) {
        case 0:
        case 1: return myArray;
        case 2:
        case 3: 
            let flip = myArray[0];
            myArray[0] = myArray[my.Array.length - 1];
            myArray[myArray.length - 1] = flip;
            return myArray;
        default:
            for (let i = 0, oddEvenAdj = myArray.length % 2, j = Math.floor(myArray.length / 2);
        i < j; i++ ) {
                let flip = myArray[i];
                let index = i + j + oddEvenAdj;
...
function arrAxis() { // Для удобства покажет серединный (статичный) элемент или
    if (myArray.length % 2 === 1) { // начало второй половины массива
        console.log('\nСередина массива: ');
