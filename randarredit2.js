...
function arrayMaxNumber() { // макс значение элемента массива
    ...
    rl.question('Введите максимальное значение элемента массива ', (answer) => {
        ...
        if (isFinite(maxNumber) && maxNumber >= -1000  && maxNumber <= 1000 && maxNumber >= arraySettings.minNumber) {
            ...
            showSet(); // Пришлось так вызывать остальные ф-ции
            arrayFill(); // т.к. иначе они все выполнялись
            showArr(); // сразу с запуском диалога с консолью
            arraySort(); // и ничего не работало
            arrAxis(); // а как иначеили лучше сделать - не знаю
            smartSort();
            showSmartArr();
        }
        else {
